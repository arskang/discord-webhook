...
var MessageBuilder = /** @class */ (function () {
    ...
    MessageBuilder.prototype.overrideWebhook = function (settings) {
        if (settings === void 0) { settings = {}; }
        if (settings.username && settings.username.trim().length > 80)
            throw new Error("username ".concat(errors_1.default.limit80));
        if (settings.avatar_url && !regexp_1.rgxURL.test(settings.avatar_url))
            throw new Error("avatar_url ".concat(errors_1.default.urlError));
        if (settings.username)
            this.message.username = settings.username;
        if (settings.avatar_url)
            this.message.avatar_url = settings.avatar_url;
        return this;
    };
